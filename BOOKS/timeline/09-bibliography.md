# Galactic Timeline — Sources and Credits

**Scoped to what *this book* actually drew on.** Other books in this set used other sources
and carry their own lists.

**⚠ Every entry carries one of three confidence levels**, and they are deliberately not
blended. A credit page is the worst place in a book to blur the line between what was
checked and what was assumed:

| Mark | Meaning |
|---|---|
| **✔ verified** | Read directly from a source held on this machine |
| **◆ relayed** | Externally researched against a live search and corroborated — **not** read from anything held here |
| **⚠ unverified** | General knowledge or assumption. Confirm before publication |

---

## Primary source

### *Star Wars: Knights of the Old Republic Campaign Guide* (2008)

**Wizards of the Coast.** **✔ Verified** — read directly from the credits page, f. 3.

**Seven of this book's eight chapters rest on this volume**, and Chapters Three through
Seven are built almost entirely from its historical sections (ff. 5–6), its dated timeline
(ff. 112–113), and its character entries (ff. 139–210).

| Role | Credited |
|---|---|
| **Design** | **Sterling Hershey, John Jackson Miller, Abel Peña, Rodney Thompson** |
| **Development** | Rodney Thompson, Stephen Radney-MacFarland, Peter Schaefer |
| **Editor** | Bradley Will |
| **Managing Editor** | Kim Mohan |
| **Design Manager** | Christopher Perkins |
| **Director of RPG R&D** | Bill Slavicsek |
| **Art Director** | Ryan Sansaver |
| **Interior artists** | Brian Snoddy, Christopher West, Drew Baker, Franz Vohwinkel, Gonzalo Flores, Jonathan Hill, Miguel Coimbra, Rebecca A. Wrigley, Steve Argyle, Steve Ellis, Udon |
| **Special thanks** | Leland Chee, Gary Sarli, Chris Tulach |

**The credits page also names thirty-four playtesters**, a production, graphic and
pre-press team, and an imaging technician. **They are not reproduced here only for length,
and the full credit belongs to them as much as to the names above** — f. 3 of the Campaign
Guide carries it in full.

**The volume builds on:** the *Star Wars Roleplaying Game Saga Edition Core Rulebook* by
**Christopher Perkins, Owen K. C. Stephens and Rodney Thompson**; the *Dungeons & Dragons*
game designed by **Jonathan Tweet, Monte Cook, Skip Williams, Richard Baker and Peter
Adkison**; and the original *Dungeons & Dragons* rules created by **E. Gary Gygax and Dave
Arneson.**

**⚠ Six names above were corrected from OCR damage** and should be checked against a
physical copy: the scan renders *Abel Peña* as "Abel Pefia", *Jonathan Tweet* as "Joriathan
Ty", *Monte Cook* as "Mante Cook", *Skip Williams* as "Skio Williams", *Richard Baker* as
"Rechard Baker", and *Peter Adkison* as "Peter Ackison". **The corrections are obvious
readings of damaged text rather than substitutions**, but they are names of real people on
a credit page, which is reason enough to verify them properly.

---

## The games

### *Star Wars: Knights of the Old Republic* (2003) — **BioWare**

Rank 1 in this project's source hierarchy. **Chapter Five's events are this game's story**,
and 3,956 BBY is its year — the campaign's default clock.

**✔ BioWare's authorship is verified from the game's own data files**, which carry the
studio's internal developer comments. **◆ The publisher
(LucasArts) and the game's creative credits are relayed** — researched externally and
corroborated, not read from anything held here.

**The full credit roll for both games lives in the Armory's front matter
(`BOOKS/armory/00-front-matter.md`) and is deliberately not duplicated here.** That book's
content is overwhelmingly these teams' design work; this one uses the games as a rank-1
cross-check. **One list, maintained in one place**, so that a correction to a credit has one place to be
made.

### *Star Wars: Knights of the Old Republic II — The Sith Lords* (2004) — **Obsidian Entertainment**

Rank 1. **Chapter Six's events are this game's story.**

**◆ Relayed.** This project reads the game's data extensively, **but that data never names
its developer, publisher or staff.** **Obsidian Entertainment, published by LucasArts**, with
**Chris Avellone** as lead designer and lead writer — researched externally and
corroborated. **Full credits in the Armory's front matter**, not duplicated here.

---

## Underlying works this book narrates without citing directly

### *Tales of the Jedi* — **Dark Horse Comics**

Rank 3 in the hierarchy. **⚠ Unverified — no creative credits for this series appear in
any source held on this machine.**

**An honest note about scope.** Chapter Three is titled *The Tales of the Jedi Era* and
tells that series' story — Exar Kun, Ulic Qel-Droma, the Krath, Nomi Sunrider, the Cron
Cluster, Yavin 4. **But every citation in that chapter is to the Campaign Guide.** The
comics themselves were not consulted, because they are not held here.

**They are credited anyway, and deliberately.** The events of one of this book's eight
chapters are that series' invention, and reaching them through a licensed summary does not
make them less so. **The creative team deserves the credit even where the citation went
through someone else's book** — and this entry is marked unverified so that whoever
finishes this page can name them properly rather than leaving it at the publisher.

---

## Consulted by relay only

### *The New Essential Chronology* (2005) — **Daniel Wallace**

Rank 6. **◆ Relayed** — researched externally and corroborated; the book is not held here
and its credits page has not been read. **⚠ If the volume has a co-author, this entry is
incomplete**, and the full credit should be taken from a physical copy before publication.

**Three claims in this entire book rest on it**, all in Chapter Two and all marked in that
chapter's text: the Force-negating plague that ended the Infinite Empire and its
approximate date, the Jedi Order's establishment on Tython, and the Battle of Corbos. The
book was **not staged**; those three facts were researched and relayed.

---

## Admissible but not used

**Recorded so a later reader does not assume they were consulted.** Both sit at **rank 8**
on the ladder Chapter One sets out — descriptive lore only, never mechanics, nothing after
3,956 BBY:

- ***Jedi vs. Sith: The Essential Guide to the Force*** (2007) — **Windham.** **⚠
  Unverified attribution.** **Never used in this book at any point.**
- ***The New Essential Guide to Alien Species*** — **⚠ unverified.** Held in
  `data/books/` and used elsewhere in this project for species lore; **not used in this
  book.**

---

## And where the sources above disagree

**Chapter One decides it, and decides it in full** — a ranked ladder, the losing reading kept
on record rather than deleted, and a worked example.

**This book reports; it does not rule.** Where one of its own pages contradicts a ranked
source about a date, **the source wins and this book has a mistake in it** — which is
Chapter One's position in its own words, and it governs this page too.

---

## ⚠ Before this book is published anywhere

**✔ One entry is fully verified:** the Campaign Guide's credits, read directly from f. 3.

**◆ Both games' studios, publishers and leads, and Daniel Wallace's attribution, are
relayed** — externally researched and corroborated, which is strong but is not the same as
having read the source.

**⚠ Three things remain genuinely unverified and should be confirmed against physical
copies:** the ***Tales of the Jedi*** creative team, **Daniel Wallace's co-authorship if
the volume has one**, and **both rank-8 titles' authors.**

**This matters more than the book's other flags.** A wrong date in Chapter Four is a
correction; a wrong or missing name on a credit page is a discourtesy to a real person who
did the work. **Flagged as the highest-priority verification item in the book.**
