...
| ⚠ **The re-lock is already an event problem** | Re-opening a step discards six fields by assignment. Under an append-only log *"a correction is a new event, not an edit"*, so a discard cannot be a deletion. **Nothing has ruled what the re-lock writes** |
| ⚠ **`backstory.lifestyle` has no writer** | It is in the record, and `PT-1401` collapsed lifestyle into the profession row, so no screen sets it |
