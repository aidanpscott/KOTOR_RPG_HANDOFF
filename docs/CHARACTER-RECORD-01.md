...
**⚠ And the next stretch is NOT "serialise the hub".** The hub holds nine choice objects **in whatever order the player clicked.** The ledger wants them as **ORDERED EVENTS** — which is a different thing, and three consequences this document already calls non-optional fall out of it.
...
```json
{
  ...
  "backstory": {
    "profession": "acolyte",
    "programming": null,
    "lifestyle": "destitute",   # ⚠ PT-1411 — ORPHANED by PT-1401
                          # lifestyle and profession are ONE field.
                          # No screen sets this; the profession row carries both
...
- ~~**`package`** — whether a character is portable between packages is not decided~~ ✅ **RULED at `PT-1415`: ONE LOG PER CHARACTER PER CAMPAIGN.** The header already pins a **rules version** (`SAVE-LOAD-01`), and `PT-1412` lets a package **close a species or class** — so a character legal in one campaign may be **illegal in another**, and replaying its creation elsewhere could produce a different character or fail. **⚠ So carrying a character across is a CHARGEN feature, not a save feature:** re-run creation under the new package's rules, seeded from the old answers. **It fails at the one screen where the player can fix it**, rather than as a wall of replay errors.
