...
- **⚠⚠ `§9`'s OWN FAILURE NODE RE-OFFERS THE CHECK, AND THAT MAKES IT FREE — `PT-1506`.** `not-on-my-board` carries `replies = ["back-off", "push"]`, so a failed Persuade returns the player to the same check **with nothing spent**: retried enough times it is passed always. That is `PT-1501`'s defect — *twelve passes out of twelve at a nominal 55%* — reached by a longer road. The example is demonstrating `§7`'s re-entrancy and the retry looks incidental to it, **but `§9` is the form every author will copy.** ⚠ **The bed's failure node was authored TERMINAL instead, and the divergence is recorded here rather than taken silently: whether a failed check may be retried is a RULE, and it is the owner's.**
