...
| Rocket | damage_over_time · condition | damage_over_time → amount · kinds · rounds · save · count? · sides? ; condition → condition · rounds · save | aimed at a target | REQUIRES A WRIST LAUNCHER, which is the gate that makes this its own row rather than a Charge. ACTION-ECONOMY-01 §6.2a: the launcher has no attack range at all — it is a launcher SLOT, not a fireable weapon, and the rocket carries the 24 m range and 20 m travel itself. Eleven items: the rockets and the darts. |
