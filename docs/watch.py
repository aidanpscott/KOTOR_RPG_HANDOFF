...
the moment a new file lands in from-designer/.
...
def ls():
    ...
    p = os.path.join(D, 'from-designer')
...
while time.time() - t0 < limit:
    ...
    if new:
        ...
        # ⚠ This printed [:4000] for four exchanges and neither agent noticed.
        # A watcher reports WHAT LANDED. Reading is a separate act, done from
        # the working tree, in full, by the agent that needs it.
        for f in sorted(new):
            p = os.path.join(D, 'from-designer', f)
